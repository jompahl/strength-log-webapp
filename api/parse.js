...
- Profile setup: {"kind":"profile","weight_kg":<num>,"height_cm":<int>,"age":<int>,"activity_level":"sedentary|light|moderate|very","target_weight_kg":<num>}
  Use this when the user is new and gives their starting stats/goals. If any required profile field is missing, set log to null and ask one brief follow-up question for the missing info.
...
module.exports = async (req, res) => {
  ...
  if (body?.onboarding) {
    userText += `[New-user setup] Your immediate job is to collect profile setup fields: current weight in kg, height in cm, age, activity level, and target weight in kg. If the user provides all of them, return a Profile setup log. If not, ask for the missing pieces.\n\n`;
  }
  if (body?.coachName) {
